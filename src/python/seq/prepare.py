...
# TODO: make this function to DBone / Worshop
...
def print_out_size(number):
    file_name = size_determine('seq/transposon/fq.size', number)
...
def check_bam(name1, name2):
    ref_md5 = pd.read_csv('seq/transposon/' + name1 + '.md5', sep=r'\s+', header=None)
    ref_md5.columns = ['md5', 'file']
    files = pd.read_csv('seq/transposon/' + name2 + '.md5', sep=r'\s+', header=None)
...
def check_md5(name):
    # define ref_md5
    ## check .fq
    ref_md5 = pd.read_csv('seq/transposon/CRA012590.csv', sep=',', header=0)
    ref_md5['file1'] = ref_md5['Read filename 1'].str.split().str[0]
    ref_md5['file2'] = ref_md5['Read filename 2'].str.split().str[0]
    # define files
    # files = pd.read_csv('seq/transposon/' + name + '.md5', sep=r'\s+', header=None)
    files = pd.read_csv('seq/transposon/' + name + '.md5', sep=r'\s+', header=None)
    files.columns = ['md5', 'file']
    ...
    for i in range(files.shape[0]):
        ...
        if line_md5.size > 0 and line_md5 == files['md5'][i]:
            ...
            print(files['file'][i] + ' is incorrect')
            # with open('seq/transposon/wrong_'+name+'.txt', 'a') as f:
        ...
        if i%2 == 1:
            if files['to_ref'][i] + files['to_ref'][i-1] == 2:
                with open('seq/transposon/pass_'+name+'.txt', 'a') as f:
                # with open('seq/transposon/pass_md5_usb.txt', 'a') as f:
                    f.write(files['file'][i].split('_')[0] + '\t' + line_sample[0] + '\n')
                pass_md5 += 1
...
def choose_v1():
    v1_size = pd.read_csv('seq/transposon/V1_size.csv', sep=',', header=0)
...
def count_bam():
    ...
    for name in ['34', '35', '36', '37', '38', '39', '40', '41']:
        with open('seq/transposon/pass_' + name + '.txt', 'r') as f:
    ...
    with open('seq/transposon/count_bam.txt', 'w', newline='') as f:
    ...
    with open('seq/transposon/ignored_bam.txt', 'w', newline='') as f:
