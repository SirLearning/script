...
// --- Assess compute (PLINK2 slice + legacy VCF/bcftools; wired from tmp/assess_*.nf) ---

/**
 * Representative-chromosome slice on existing per-subgenome `*_test.plink2` under
 * `process/<mod>/`. Emits PLINK2 .acount (allele counts from `--freq counts`) / .vmiss,
 * site-count TSV, MAF-bin histogram from derived MAF, and a GQ placeholder (FORMAT/GQ
...
process plink2_assess_debug_slice {
    ...
    publishDir "${params.output_dir}/${params.job}/process/${params.mod}/variant", mode: 'copy', pattern: "*.acount"
    publishDir "${params.output_dir}/${params.job}/process/${params.mod}/variant", mode: 'copy', pattern: "*.vmiss"
    ...
    NR==1 { next }
    {
        alt=\$5+0
        obs=\$6+0
        if (obs <= 0 || \$5 == "." || \$6 == ".") { b["maf_missing"]++; next }
        ref = obs - alt
        if (ref < 0) { b["maf_missing"]++; next }
        mac = (alt < ref) ? alt : ref
        maf = mac / obs
        if (maf <= 0) b["maf_eq_0"]++
    ...
    }' ${id}.assess.acount > ${id}.mac_site_histogram.tsv
...
process quick_count {
    tag "quick count: ${id}"
    conda "${params.user_dir}/miniconda3/envs/stats"
    publishDir "${params.output_dir}/${params.job}/process/${params.mod}/info", mode: 'copy'
