 ...
 * Invoked from main.nf via RUN_WHEAT_INTEGRATED or partial_router.nf (--partial_task wheat_from_plink).
 */
...
include {
    ...
    mk_plink_basic_info
    awk_depth_cnv_call
} from '../../../modules/local/genotype/processor/processor_plink2.nf'
include {
    ...
    plot_genetic_map
    report_hapmap_table
} from '../../../modules/local/genotype/stats/stats_integrated.nf'
include { plink2_gwas_glm; gcta_gwas } from '../../../modules/local/static/gwas/gwas_plink2.nf'
include { plot_gwas_association } from '../../../modules/local/static/gwas/gwas_plot.nf'

workflow RUN_WHEAT_FROM_PLINK {
    main:
    if (!(params.mod in ['test_thin', 'test_common_thin'])) {
        error "RUN_WHEAT_FROM_PLINK: params.mod must be test_thin or test_common_thin (got: ${params.mod})."
    }
