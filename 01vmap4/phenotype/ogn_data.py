...
def org_China(China_data, file_name):
    file_data = pd.read_csv('phenotype/transposon/Watseq/China_phenotype/' + file_name, sep='\t', header=0)
    if China_data.empty:
...
def check_trait(China_data):
    ref_trait = pd.read_csv('phenotype/transposon/Watseq/trait_information.csv', sep=',', header=0)
    ...
    for trait in ref_trait['Abbreviation used in plots']:
        if trait not in [col.split('_')[0] for col in China_data.columns]:
            print(trait + ' is not in the China transposon')
            continue
        measured_num += 1
...
def main():
    ...
    # print(len(os.listdir('phenotype/transposon/Watseq/China_phenotype')))
    for file_name in os.listdir('phenotype/transposon/Watseq/China_phenotype'):
        China_data = org_China(China_data, file_name)
    # China_data.to_csv('phenotype/transposon/Watseq/China_data_output.csv', index=False)
