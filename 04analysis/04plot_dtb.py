...
def plot_te_hist(axs, file):
    ...
    axs.hist(density['win_end'], weights=density['density'], bins=density['win_num'].iloc[-1], alpha=0.8, label=file)
...
for i in range(0, len(file_name)-1):
    plot_te_hist(ax, file_name.iloc[i, 0])
