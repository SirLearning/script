...
def plot_te_hist(axs, file, alpha, method):
    file = str(file)
    density = pd.read_table('transposon/distribution/' + method + '.' + file + '.density.txt', sep='\s+', header=None)
...
def te_plot(axs, file, chrom, i):
    file = str(file)
    density = pd.read_table('transposon/' + chrom + '/stats.' + file + '.dtb.txt', sep='\s+', header=None)
    density.columns = ['chrom', 'win_start', 'win_end', 'win_num', 'none', 'strand', 'density']
...
def compare_te_hist(axs, file, method):
    file = str(file)
    density = pd.read_table('transposon/distribution/' + method + '/' + method + '.' + file + '.density.txt', sep='\s+', header=None)
    density.columns = ['chrom', 'win_start', 'win_end', 'win_num', 'none', 'strand', 'density']
...
def compare_te(axs, file, name, method, color, fig_type):
    file = str(file)
    density = pd.read_table('transposon/old/' + method + '/stats.' + file + '.dtb.txt', sep='\s+', header=None)
    density.columns = ['chrom', 'win_start', 'win_end', 'win_num', 'none', 'strand', 'density']
    if fig_type == 'hist':
...
def difference_te_hist(axs, file, method):
    file = str(file)
    density = pd.read_table('transposon/' + method + '/chr1ANM.' + file + '.density.txt', sep='\s+', header=None)
    density.columns = ['chrom', 'win_start', 'win_end', 'win_num', 'none', 'strand', 'density']
...
def genome_dtb(axs, spc, name, method, i):
    cover = pd.read_table('transposon/' + spc + '/' + method + '/coverage.' + name + '.dtb.txt', sep='\s+', header=None)
...
def cs_dtb(axs, spc, name, chr, i):
    cover = pd.read_table('transposon/' + spc + '/coverage.' + name + '.dtb.txt', sep='\t', header=None)
    cover.columns = ['chrom', 'win_start', 'win_end', 'win_num', 'none', 'strand', 'coverage']
...
def plot_length(name):
    length = pd.read_table('transposon/old/' + name + '/stats.length.txt', sep='\t')
